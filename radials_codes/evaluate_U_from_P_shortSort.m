...
short_samples = 2048; % number of measurements taken from original measurements

num_splits = round(T_meas / (T_chirp * short_samples / 60));
H = num_splits / average_every;
dt = T_chirp * short_samples / 60;

basic_path = 'Z:\radials_spectrum_shortSort\short_2048_shift_2048_range_100\is1\R_20.9184_ang_22_22\';
...
for cur_split = 1 : length(files)
    ...
    [cur_U_all, res, sig, acc, f_peaks] = get_U_shortSort(cur_filename, '', 'centroid');
